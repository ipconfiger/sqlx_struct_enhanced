...
async fn test_something_async() -> Result<(), sqlx::Error> {
    ...
    sqlx::query("DELETE FROM test_tb WHERE id = 'test-1-asdasd'").execute(&pool).await.ok();

    let mut bar = TestTb{id:"test-1-asdasd".to_string(), name:"asdasd".to_string(), ts:1231312};
    bar.insert_bind().execute(&pool).await?;
    let tb = TestTb::by_pk().bind("test-1-asdasd").fetch_one(&pool).await?;
...
async fn test_bulk_operations() -> Result<(), sqlx::Error> {
    ...
    // Clean up before test
    sqlx::query("DELETE FROM test_tb WHERE id LIKE 'test-2-bulk-%'").execute(&pool).await.ok();

    // Test bulk insert
    let items = vec![
        TestTb { id: "test-2-bulk-1".to_string(), name: "Alice".to_string(), ts: 100 },
        TestTb { id: "test-2-bulk-2".to_string(), name: "Bob".to_string(), ts: 200 },
        TestTb { id: "test-2-bulk-3".to_string(), name: "Charlie".to_string(), ts: 300 },
    ];
    TestTb::bulk_insert(&items).execute(&pool).await?;

    // Test bulk select
    let ids = vec!["test-2-bulk-1".to_string(), "test-2-bulk-2".to_string(), "test-2-bulk-3".to_string()];
    ...
    assert!(result_ids.contains(&"test-2-bulk-1"));
    assert!(result_ids.contains(&"test-2-bulk-2"));
    assert!(result_ids.contains(&"test-2-bulk-3"));

    // Test bulk select with different order (order not guaranteed)
    let ids_reordered = vec!["test-2-bulk-3".to_string(), "test-2-bulk-1".to_string(), "test-2-bulk-2".to_string()];
    ...
    assert!(reordered_ids.contains(&"test-2-bulk-1"));
    assert!(reordered_ids.contains(&"test-2-bulk-2"));
    assert!(reordered_ids.contains(&"test-2-bulk-3"));

    // Note: bulk_update temporarily skipped due to type compatibility issue
    // TODO: Fix bulk_update to handle mixed field types correctly

    // Test bulk delete
    let ids_to_delete = vec!["test-2-bulk-1".to_string(), "test-2-bulk-2".to_string(), "test-2-bulk-3".to_string()];
...
async fn test_where_and_delete_queries() -> Result<(), sqlx::Error> {
    ...
    // Insert test data
    let items = vec![
        TestTb { id: "test-3-where-test-1".to_string(), name: "Test1".to_string(), ts: 10 },
        TestTb { id: "test-3-where-test-2".to_string(), name: "Test2".to_string(), ts: 20 },
        TestTb { id: "test-3-where-test-3".to_string(), name: "Test3".to_string(), ts: 30 },
    ];
    TestTb::bulk_insert(&items).execute(&pool).await?;
    ...
    let (remaining_count,) = TestTb::count_query("id LIKE 'test-3-where-test-%'").fetch_one(&pool).await?;
