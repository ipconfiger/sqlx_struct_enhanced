// SQLite Integration Test - Binary Program
//
// This is a standalone binary program to test SQLite integration with
// all extended BindProxy data types.
...
async fn get_test_pool() -> SqlitePool {
    let database_url = std::env::var("SQLITE_DATABASE_URL")
        .unwrap_or_else(|_| "sqlite::memory:".to_string());

    println!("🔧 Connecting to SQLite: {}", database_url);
    ...
    match sqlx::SqlitePool::connect(&database_url).await {
        ...
        Err(e) => {
            panic!("❌ Failed to connect to SQLite test database: {}", e);
        }
    }
}

// Helper function to create test table with all supported types
async fn create_extended_types_table(pool: &SqlitePool) {
    ...
    println!("✅ Table 'extended_types_test' created");
}
...
async fn test_sqlite_extended_types_insert_select_numeric(pool: &SqlitePool) {
...
async fn test_sqlite_extended_types_chrono_datetime(pool: &SqlitePool) {
    println!("\n🔧 Starting test_sqlite_extended_types_chrono_datetime...");
...
async fn test_sqlite_extended_types_binary(pool: &SqlitePool) {
    println!("\n🔧 Starting test_sqlite_extended_types_binary...");
...
async fn test_sqlite_extended_types_uuid(pool: &SqlitePool) {
    println!("\n🔧 Starting test_sqlite_extended_types_uuid...");
...
async fn test_sqlite_extended_types_json(pool: &SqlitePool) {
    println!("\n🔧 Starting test_sqlite_extended_types_json...");
...
async fn test_sqlite_extended_types_complex_where(pool: &SqlitePool) {
    println!("\n🔧 Starting test_sqlite_extended_types_complex_where...");
...
async fn test_sqlite_extended_types_unsigned_where(pool: &SqlitePool) {
    println!("\n🔧 Starting test_sqlite_extended_types_unsigned_where...");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 SQLite Integration Tests - Binary Program");
    println!("==========================================\n");
    ...
    // Run all tests
    test_sqlite_extended_types_insert_select_numeric(&pool).await;
    test_sqlite_extended_types_chrono_datetime(&pool).await;
    test_sqlite_extended_types_binary(&pool).await;
    test_sqlite_extended_types_uuid(&pool).await;
    test_sqlite_extended_types_json(&pool).await;
    test_sqlite_extended_types_complex_where(&pool).await;
    test_sqlite_extended_types_unsigned_where(&pool).await;

    println!("\n==========================================");
    println!("✅ All SQLite integration tests passed!");
    println!("==========================================\n");
