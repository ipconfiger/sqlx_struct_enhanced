// MySQL Integration Test - Binary Program
//
// This is a standalone binary program to avoid Cargo feature resolution
// issues with derive macros in workspace test files.
...
async fn get_test_pool() -> MySqlPool {
    ...
    for i in 0..10 {
        match sqlx::MySqlPool::connect(&database_url).await {
            ...
            Err(e) => {
                println!("⚠️  Attempt {} failed: {}", i + 1, e);
...
async fn test_mysql_extended_types_insert_select_numeric(pool: &MySqlPool) {
...
async fn test_mysql_extended_types_chrono_datetime(pool: &MySqlPool) {
...
async fn test_mysql_extended_types_binary(pool: &MySqlPool) {
...
async fn test_mysql_extended_types_uuid(pool: &MySqlPool) {
...
async fn test_mysql_extended_types_json(pool: &MySqlPool) {
...
async fn test_mysql_extended_types_complex_where(pool: &MySqlPool) {
...
async fn test_mysql_extended_types_unsigned_where(pool: &MySqlPool) {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 MySQL Integration Tests - Binary Program");
    println!("==========================================\n");
    ...
    // Run all tests
    test_mysql_extended_types_insert_select_numeric(&pool).await;
    test_mysql_extended_types_chrono_datetime(&pool).await;
    test_mysql_extended_types_binary(&pool).await;
    test_mysql_extended_types_uuid(&pool).await;
    test_mysql_extended_types_json(&pool).await;
    test_mysql_extended_types_complex_where(&pool).await;
    test_mysql_extended_types_unsigned_where(&pool).await;
